...
- **Rules:**
  - **Length:** Length >= 8 chars → (+1 point). Else: feedback "Password is too short".
  - **Number:** Contains digit (0-9) → (+1 point). Else: feedback "Add a number".
  - **Uppercase:** Contains uppercase (A-Z) → (+1 point). Else: feedback "Add an uppercase letter".
  - **Special:** Contains special char (!@#$%^&*) → (+1 point). Else: feedback "Add a special character".
...
- **Condition:** Missing field (`password` key) or Null value (`"password": null`).
