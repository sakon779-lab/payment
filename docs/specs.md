...
- Start at Score = 0. Max Score = 4.
- Add points only. Do NOT subtract points. Minimum score is 0.

##### Rules:
1. **Length**:
   - Length >= 8 chars → (+1 point). Else, add feedback: "Password is too short".
2. **Number**:
   - Contains at least one digit (0-9) → (+1 point). Else, add feedback: "Add a number".
3. **Uppercase**:
   - Contains at least one uppercase letter (A-Z) → (+1 point). Else, add feedback: "Add an uppercase letter".
4. **Special**:
   - Contains at least one special char (!@#$%^&) → (+1 point). Else, add feedback: "Add a special character".
